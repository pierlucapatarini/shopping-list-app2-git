...
function ArchivioDocumenti() {
    ...
    useEffect(() => {
    if (location.state && location.state.file_url) {
        setIsImageLoading(true);
        const { file_url, file_name, file_type } = location.state;
        ...
        fetch(file_url)
            ...
            .then(blob => {
                const fileFromBlob = new File([blob], file_name, { type: file_type });
                setFile(fileFromBlob);
                setEditableFileName(file_name.split('.').slice(0, -1).join('.')); // togli l’estensione
            })
            .catch(err => {
                console.error("Errore nel recupero del file dalla chat:", err);
                setError("Impossibile caricare il file dalla chat. Si prega di provare a caricarlo manualmente.");
                setFile(null);
            })
            .finally(() => {

...
import '../styles/archivio.css'; // Assicurati di avere questo file CSS

const locales = { it };
const localizer = dateFnsLocalizer({ format, parse, startOfWeek, getDay, locales });

// L'unica categoria è 'FARMACO'
const categoryColors = { 'FARMACO': '#33FFF9' };

// Componente per la vista degli eventi nel calendario
...
// Toolbar personalizzata rimane invariata
        ...
        try {
            if (formData.repeatPattern === 'nessuna') {
                ...
                const { data, error } = modalData?.id 
                    ? await supabase.from('events_farmaci').update(eventData).eq('id', modalData.id).select()
                    : await supabase.from('events_farmaci').insert([eventData]).select();
        ...
        try {
            ...
            if (id) {
                ...
                const { data: insertData, error: insertError } = await supabase
                    .from('ArchivioFarmaci')
                    .insert([{ ...itemDataToSave, family_group: profile.family_group }])
                    .select();
                data = insertData;
                error = insertError;
            }
                                ...
                                <tbody>
                                    {archivioFarmaci.map(item => (
                                        <tr key={item.id} className={item.quantita_attuale <= item.quantita_scortaminima ? 'low-stock' : ''}>
                                            ...
                                            <td>
                                                <button onClick={() => { setShowMagazzinoModal(true); setMagazzinoFormData(item); }}>Modifica</button>

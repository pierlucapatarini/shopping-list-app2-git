...
function FamilyChat() {
    ...
    const setupPushNotifications = async (currentUser, currentFamilyGroup) => {
        // Aggiunto controllo per assicurarci che i dati dell'utente siano disponibili prima di procedere.
        if (!currentUser || !currentFamilyGroup) {
            console.log("Dati utente non ancora disponibili per le notifiche.");
            return;
        }
        ...
        try {
            ...
            const { error } = await supabase
                ...
                .upsert({
                    user_id: currentUser.id, // Corretto da `user.id` a `currentUser.id`
                    family_group: currentFamilyGroup, // Aggiunto per tracciare il gruppo familiare
                    subscription: subscription
                }, { onConflict: ['user_id'] }); // Corretto per usare 'user_id' come campo di conflitto
    ...
    useEffect(() => {
        ...
        const initChatAndPresence = async () => {
            ...
            try {
                ...
                // Recupero il gruppo familiare dal metadata dell'utente
                const fg = user.user_metadata.family_group;
                if (!fg) return;
                setFamilyGroup(fg);

                // *** CHIAMATA A setupPushNotifications SPOSTATA QUI ***
                // Chiamiamo la funzione solo dopo che l'utente e il familyGroup sono stati caricati
                await setupPushNotifications(user, fg);
                // ****************************************************
    ...
    const handleSendMessage = async (e) => {
        e.preventDefault();
        // Aggiunto controllo robusto per user e familyGroup
        if ((!newMessage.trim() && !selectedFile) || !user || !familyGroup || uploadingFile) {
            console.error("Errore: Dati utente o gruppo mancanti.");
            return;
        }

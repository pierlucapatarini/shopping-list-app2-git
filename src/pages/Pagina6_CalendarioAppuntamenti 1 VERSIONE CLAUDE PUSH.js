...
// Chiave VAPID pubblica (dovrai generarla per la tua app)
const VAPID_PUBLIC_KEY = 'TUA_CHIAVE_VAPID_PUBBLICA_QUI';
...
export default function Pagina6() {
    ...
    const initializePushNotifications = async () => {
        ...
        if ('serviceWorker' in navigator && 'PushManager' in window) {
            ...
            console.log('Push messaging non supportato');
    ...
    const testPushNotification = async () => {
        ...
        try {
            // Chiama la funzione Edge di Supabase per inviare la notifica
            const { data, error } = await supabase.functions.invoke('send-push-notification', {
                body: {
                    familyGroup: profile.family_group,
                    title: '🧪 Test Notifica',
                    body: `Test notifica inviata da ${profile.username}`,
                    icon: '/icon-192.png',
                    badge: '/icon-72.png',
                    ...
                    {pushSupported && (
                        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                            {notificationPermission !== 'granted' ? (
                                <button onClick={requestNotificationPermission} className="btn-warning">
                                    🔔 Abilita Notifiche
                                </button>
                            ) : (
                                <button onClick={testPushNotification} className="btn-success">
                                    🧪 Test Notifica Push
                                </button>
                            )}
                            <span style={{ fontSize: '12px', color: '#666' }}>
                                {notificationPermission === 'granted' ? '✅ Attive' : 
                                 notificationPermission === 'denied' ? '❌ Negate' : '⏳ In attesa'}
                            </span>
                        </div>
                    )}
                </div>
                            ...
                            <div className="modal-body">
                                ...
                                {formData.isNotificationEnabled && (
                                    <div className="notification-container">
                                        ...
                                        <div className="form-group">
                                            <label>Invia notifica push a:</label>

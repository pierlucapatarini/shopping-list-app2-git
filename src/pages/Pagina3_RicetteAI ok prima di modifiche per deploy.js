...
const addItemToShoppingList = async (product) => {
    ...
    // FUNZIONE PRINCIPALE: ANALIZZA LA RICETTA E TROVA GLI INGREDIENTI
        ...
        try {
            const chatCompletion = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "Sei un assistente esperto in ricette e shopping. Il tuo compito è estrarre gli ingredienti da una ricetta data dall'utente. Rispondi solo con una lista di ingredienti separati da una virgola, senza altre frasi o spiegazioni. Non includere le quantità o le istruzioni, solo il nome dell'ingrediente. Ad esempio: 'pomodoro, mozzarella, basilico'."
                    },
                    {
                        role: "user",
                        content: recipeText
            ...
            const extractedIngredients = rawIngredients.split(',').map(name => ({
                name: name.trim(),
                checked: false
            }));
            ...
            console.error("Errore nell'analisi della ricetta con OpenAI:", error);
            alert(`Si è verificato un errore durante l'analisi: ${error.message}`);
            setLoading(false);
        }
    };

    // FUNZIONE PER OTTENERE ISTRUZIONI DI COTTURA + METEO + BARZELLETTE
    const handleGetCookingInstructions = async () => {
        ...
        try {
            ...
            console.error("Errore nel recupero delle istruzioni di cottura con OpenAI:", error);
            alert(`Si è verificato un errore durante l'analisi: ${error.message}`);
            setLoading(false);
        }
    };
